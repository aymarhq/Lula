...
O frontend envia `POST` JSON com `{ image, scenario, seal, dedication, prompt }`, onde `image` é um data URL. O endpoint deve responder `{ "image": "data:image/png;base64,..." }` ou `{ "imageUrl": "https://..." }`. Assim o navegador nunca acessa diretamente a credencial do gerador.
